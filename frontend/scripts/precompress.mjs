...
const root = new URL('../dist/admin-assets/', import.meta.url);
for (const entry of await readdir(root, { withFileTypes: true })) {
  ...
  await Promise.all([writeFile(new URL(entry.name + '.gz', root), gzip), writeFile(new URL(entry.name + '.br', root), brotli)]);
  console.log(`${entry.name}: ${source.length} → gzip ${gzip.length}, br ${brotli.length} bytes`);
}
