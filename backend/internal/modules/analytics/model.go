...
const eventWhere = " WHERE e.occurred_at >= $1 AND e.occurred_at < $2 AND ($3::bigint=0 OR e.link_id=$3) AND ($4::text='' OR e.ad_id=$4) "

func (f Filter) args() []any { return []any{f.Start, f.End, f.LinkID, f.AdID} }
...
const eventCols = "e.id,e.occurred_at,e.link_id,l.code,coalesce(e.visitor_id,''),e.cookie_status,e.method,e.device,e.os,e.browser,e.country,e.region,e.city,e.source,e.ad_id,e.classification,e.reason,e.referrer,e.attribution_conflict,e.event_type,e.whatsapp_clicked_at,e.auto_redirected_at"
